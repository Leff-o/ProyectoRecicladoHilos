...
public class Caja {
    ...
    public synchronized void cobra(Usuario usuario) throws InterruptedException {
        ...
        if (arrayCaixes[usuario.getNumeroCajaUsuario()] == false) {    // Si la caja no está en uso
            arrayCaixes[usuario.getNumeroCajaUsuario()] = true;    // La ponemos como que está en uso
            ...
            ganancias = usuario.getDineroGastado();  // Asignamos a la posición del array correspondiente el dinero que ha gastado el cliente
            numeroVentas++; // Incrementamos en uno el numero de ventas
            arrayCaixes[usuario.getNumeroCajaUsuario()] = false;   // Se ha cobrado al cliente así que vuelve a estar disponible
        }
    }
